...
def write_manifest(outputs: Path, rows, totals, dest: Path):
    ...
    if empty:
        lines += [
            ...
            "- `deepseek-coder-v2-lite` — dropped. Its bundled modelling code targets",
            "  transformers ~4.38 and fails four distinct ways on anything newer:",
            "  a missing is_torch_fx_available, a renamed DynamicCache.seen_tokens,",
            "  a renamed get_usable_length, and finally a tensor-shape mismatch in",
            "  its attention mask. The first three were shimmed; the fourth is a",
            "  numerical difference, not a rename, and a shim there could be",
            "  silently wrong rather than merely absent. Running it would need an",
            "  environment pinned to its original transformers, which is a",
            "  different experiment setup from every other model here.",
